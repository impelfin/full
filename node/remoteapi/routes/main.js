...
app.get('/Hello', (req, res) => {
  urls = 'http://54.151.233.199:8000/Hello';
...
app.get('/api/users', (req, res) => {
  axios
    .get('http://54.151.233.199:8000/api/users')
    .then(result => {
...
app.get('/api/users/user', (req, res) => {
  if (req.query.name == null) {
    urls = 'http://54.151.233.199:8000/api/users/user?user_id=' + req.query.user_id;
  } else {
    urls = 'http://54.151.233.199:8000/api/users/user?user_id=' + req.query.user_id + '&name=' + req.query.name;
  }
  request(urls, { json: true }, (err, result, body) => {
    if (err) {
      return console.log(err);
...
app.get('/api/users/:user_id', (req, res) => {
  urls = 'http://54.151.233.199:8000/api/users/' + req.params.user_id;
  request(urls, { json: true }, (err, result, body) => {
    if (err) {
      return console.log(err);
    }
...
app.post('/api/users/userBody', (req, res) => {
  const option = {
    uri: 'http://54.151.233.199:8000/api/users/userBody',
    method: 'POST',
    form: { id: req.body.id },
  };
...
app.post('/api/users/add', (req, res) => {
  const option = {
    uri: 'http://54.151.233.199:8000/api/users/add',
    method: 'POST',
...
app.put('/api/users/update', (req, res) => {
  const option = {
    uri: 'http://54.151.233.199:8000/api/users/update',
    method: 'PUT',
    form: { id: req.body.id, name: req.body.name },
  };
...
app.patch('/api/users/update/:user_id', (req, res) => {
  const option = {
    uri: 'http://54.151.233.199:8000/api/users/update/' + req.params.user_id,
    method: 'PATCH',
...
app.delete('/api/users/delete', (req, res) => {
  const option = {
    uri: 'http://54.151.233.199:8000/api/users/delete',
    method: 'DELETE',
